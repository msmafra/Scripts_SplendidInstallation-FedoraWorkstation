...
Export_Nautilus-Scripts() {
    ...
    for FILE in $HOME/.local/share/nautilus/scripts/*.sh
    do
        if [[ -f $WORKINGDIRECTORY/Nautilus/$FILE ]]
        then
            read -p "Would you like to overwrite last exported Nautilus script \"$FILE\"? (y/ anything else to skip): "

            if [[ $REPLY =~ ^[Yy]$ ]]
            then
                cp $FILE $WORKINGDIRECTORY/Nautilus
            else
                echo "Exporting Nautilus script \"$FILE\" skipped!"
            fi
        else
            cp $FILE $WORKINGDIRECTORY/Nautilus
        fi
...
Export_Settings-GNOME() {
    ...
    then
        ...
        if [[ $REPLY =~ ^[Yy]$ ]]
        then
            GNOMESETTINGS="Settings-GNOME_Exported.dconf"
        else
