...
Configure_Users() {
    ...
    read -p "Type the administrators' password: " "ROOTSPASSWORD"
    ...
    read -p "Type the users' name and surname: " USERSNAME USERSSURNAME
...
Configure_DiskPartitioning() {
    ...
    # Modifying users' Btrfs subvolume name
    # Not asking for a users' name twice if a users configuring script has been ran
    if [[ -z $USERSNAME ]]
    then
        read -p "Type the users' name: " USERSNAME
    fi
