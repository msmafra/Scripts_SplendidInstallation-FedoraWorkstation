...
Install_SoftwareFromRepositories-Pre-selected() {
    ...
    sudo dnf install --assumeyes kernel-devel kernel-headers akmods
    ...
    # sudo dnf install --assumeyes bluefish
...
Install_SoftwareFromRepositories-Listed() {
    ...
    read -p "Would you like to load a pre-selected or a custom list of software form repositories? (p/c/ anything else to skip): "
